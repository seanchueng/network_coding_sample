/*
 * 程序名：poll_model.cpp，此程序用于演示采用poll模型实现网络通讯的服务端。
*/
#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
...
int main(int argc,char *argv[])
{
    ...
    pollfd fds[2048];                 // fds存放需要监视的socket。pollfd是一个结构体。可以自定义数组的大小。

    /* struct pollfd{
        int fd;         //文件(或者套接字)描述符 ，即需要监视的socket。
        short events;   //需要监视的事件
        short revents;  //poll()返回的事件。如果有事件发生，poll()只会修改该成员。
    }
    
    */


    // 初始化数组，把全部的socket设置为-1，如果数组中的socket的值为-1，那么，poll将忽略它(这是poll()的特点)。
    for (int ii=0;ii<2048;ii++)             
        fds[ii].fd=-1;   

    // 打算让poll监视listensock读事件。
    fds[listensock].fd=listensock;     //即对应位置上置为索引。但是会忽略fds[0];
    fds[listensock].events=POLLIN;        // POLLIN表示读事件，POLLOUT表示写事件。可以通过 man poll()查看。
    // fds[listensock].events=POLLIN|POLLOUT;   //通过或运算同时处理读写事件。
    ...
    while (true)        // 事件循环。
    {
        ...
        int infds=poll(fds,maxfd+1,10000);      // 超时时间为10秒。第一个参数是结构体的地址。第三个参数若为0，表示不接受延时，poll函数立即返回。
