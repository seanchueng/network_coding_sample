...
int main(int argc,char *argv[])
{
  ...
  servaddr.sin_addr.s_addr = htonl(INADDR_ANY); // 如果服务器有多个网卡，INADDR_ANY参数表示服务端任意网卡的IP都可以用于通讯。这是<arpa/inet.h>库中的函数。该方法也有局限性，即不能使用主机名或者域名。
//servaddr.sin_addr.s_addr = inet_addr("192.168,101,138") ； // 指定服务器用于通信的IP(大端序);
