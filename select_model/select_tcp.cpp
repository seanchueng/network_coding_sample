/*
 * 程序名：select.cpp，此程序用于演示采用select模型实现网络通讯的服务端。
*/
#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
...
int main(int argc,char *argv[])
{
    ...
    //把用于监听的socket加入到select的集合中。
    // fd_set 本质上是int[32] 8*4*32=1024bit 的bitmap(位图)
    ...
    int maxfd=listensock;              // readfds中socket的最大值。用于减少轮询bitmap的次数。
